<script>
	...
	const dismiss = (
		/** @type {{ preventDefault: () => void; dataTransfer: { getData: (arg0: string) => string; }; }} */ e
	) => {
		e.preventDefault();
	...
	const loadTimeline = (/** @type {{ data?: any; }} */ timeline) => {
		if (timeline?.data) {
			...
			if (timeline.data.style) {
				({ backgroundColor, textColor, fontFamily, fontSize, nTicks, ffName } =
					timeline.data.style);
			} else {
				//reset to defaults
				backgroundColor = '#efefef';
			...
			//timeline undefined, reset all the things.
			resetStyle();
		}
	};
...
<div class="container" bind:clientHeight={height}>
	<div class="sidebar left">
		...
		<div class="results" on:dragover={(e) => dragover(e, '')} on:drop={dismiss}>
			{#await results then docs}
				...
				{#each docs.search.results as document}
					{#if document.date && isNotSelected(document.id, selected) && isNotTrash(document.id, trashed)}
						<div class="result" draggable="true" on:dragstart={(e) => dragFromResult(e, document)}>
	...
	<div class="main" on:drop={addToTimeline} on:dragover={dragover} bind:this={main}>
