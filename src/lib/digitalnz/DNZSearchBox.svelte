<script>
	import {
		...
		categories,
		primaryCollections
	} from '$lib/digitalnz/digitalNZutils.js';
	...
	$: collections = primaryCollections(category).then(updateCollections);
...
<fieldset>
	...
	<div class="input">
		...
		<select id="collection" bind:value={primary_collection}>
	...
	{#if showHasDate}
		<div class="checkbox">
			<input type="checkbox" id="hasdate" bind:checked={hasDate} label="hasdate" />
			<label for="hasdate">Has date</label>
		</div>
	{/if}
	<div class="input">
		<label for="term">Search Term</label>
		<input type="text" id="term" bind:value={text} label="term" on:keypress={enter} />
	</div>
	<button on:click={search}>Go!</button>
	<div class="attribution">
		<p>
			This website uses data sourced from <a href="http://www.digitalnz.org" target="_blank"
				>DigitalNZ</a
			>.
		</p>
