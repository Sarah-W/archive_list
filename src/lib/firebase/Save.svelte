<script>
	...
	const save = async () => {
		const docs = Object.values(documents).map(({ id, height, width, x, y }) => ({
			id,
			height,
			width,
			x,
...
<fieldset>
	...
	{#if $user}
		<div class="input" style:display={makeNewTimeline ? 'none' : null}>
			...
			<select type="select" id="timelines" bind:value={index} on:change={load}>
			...
			<div style:display={makeNewTimeline ? 'none' : null} class="buttonset">
				...
				<button disabled={index == 'blank'} on:click={togglePublish}
					>{$list[index]?.data.published ? 'Unpublish' : 'Publish'}</button
				>
		...
		<div class="input" style:display={makeNewTimeline ? null : 'none'}>
			...
			<input type="text" id="timelinename" bind:value={newTimelineName} />
